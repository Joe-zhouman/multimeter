...
namespace multimeter
{
    public partial class SetupTest {
        public bool apply_btm() {
            string filePath = IniReadAndWrite.IniFilePath;
            switch (_method) {
                case TestMethod.KAPPA: 
                    if (!apply_btm_1_Click()) {
                        return false;
                    }
                    break;
                case TestMethod.ITC:
                    if (!apply_btm_2_Click()) {
                        return false;
                    }
                   
                    break;
                case TestMethod.ITM:
                    if (!apply_btm_3_Click()) {
                        return false;
                    }
                    
                    break;
                case TestMethod.ITMS:
                    if (!apply_btm_4_Click()) {
                        return false;
                    }
                    break;
                default: {
        ...
        private bool apply_btm_2_Click()
        {
            if (User == UserType.ADVANCE)
            {
                ...
                if(!BoxToSpecimen(ref _device.HeatMeter1, heatMeterPositionBoxes1, heatMeterChannelBoxes1, K1TextBox2_1, S1TextBox2_1))return false;
                List<TextBox> heatMeterPositionBoxes2 = new List<TextBox>()
        ...
        private bool apply_btm_3_Click()
        {
            if (User == UserType.ADVANCE) {
                ...
                if (!BoxToSpecimen(ref _device.HeatMeter1, heatMeterPositionBoxes1, heatMeterChannelBoxes1, K1TextBox3_1,
